...
Ajax smart keyword suggestions and fix typos for better results by showing "Did You Mean", Google style! Simple, Minimal and Fast.

== Description ==
QMean is a minimal auto-complete query and 'did you mean' suggestion plugin which search for relative keywords and fix typos in words. Its goal is to help users to create relevant queries before sending it to WP search engine.
...
QMean will search through database and collect keywords and phrases related to the given input. You are the one who decides where to look: Titles, Excerpts, Contents or PostMetas and on which post types.
...
You can position the box as you wish via settings and change the search engine from there too.

== Key Features: ==

* Live query suggestions
* Phrase Mode or Word By Word Searching
* Search through titles, excerpts, contents or even none hidden post metas
...
Both of them are useful, it depends on your need. Phrase mode can have solid results at the end but fewer suggestions. But WBW can have more suggestions and fewer results because it will suggest the most related keyword from the entire database! it can be useful to optimize your SEO through (if user created that phrase and you didn't predicted it, it is not QMeans's fault :) )
