	...
	$(document).on('click',function(e){
		...
		if(tag_name != 'INPUT' && tag_name != 'TEXTAREA'){
			...
			if(qmean_tooltip.find('.error').length > 0){
				qmean_tooltip.find('.error').html('<strong>'+tag_name+'</strong>'+qmean.labels.isNotValid+' '+qmean.labels.pleaseChooseAnInputType);
			} else {
				qmean_tooltip.append('<span class="error">'+'<strong>'+tag_name+'</strong>'+qmean.labels.isNotValid+' '+qmean.labels.pleaseChooseAnInputType+'</span>');
			}
			...
			if( elm_id !== undefined && elm_id != '' ){
				...
				if(parent_id !== undefined && parent_id != ''){
					...
					selector = '.'+parent_class.trim().split(" ").join('.')+' .'+selector;
				}
			}
			...
			qmean_tooltip.after('<div id="qmean-finalize-recognized-selector" class="qmean-skip-click"><span class="qmean-skip-click">'+qmean.labels.yourSelectorIs+': <strong class="qmean-skip-click">'+selector+'</strong></span><button id="qmean-save-recognizer" class="qmean-skip-click">'+qmean.labels.saveSelector+'</button></div>');
		}
	});
